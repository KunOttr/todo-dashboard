...
- **防限速自动同步**：所有操作先在本地生效并入队，**距最后一次操作 5 分钟**后自动调用 API 批量保存；「同步」按钮实时显示待同步数量与剩余倒计时，点击立即保存。保存成功后为**增量同步**——直接用服务器返回的最新数据更新本地状态，不再整页刷新（仅同步失败时才回退全量拉取）。
...
3. 首次连接会自动在仓库中创建系统标签：`归档`；进度标签（`进度:0%` ~ `进度:100%`）改为**惰性创建**——首次给待办设置进度时才自动补建缺失的对应标签，不再一次性预建 11 个。
...
- **平台专属功能**：受平台 API 能力限制的功能一律作为**平台专属功能**实现，不做跨平台的降级模拟。某项能力在某个平台不可用时，该项直接不展示（其余功能不受影响）；可用范围由仓库配置的 **API 格式**（GitHub GraphQL / Gitea REST）决定。
